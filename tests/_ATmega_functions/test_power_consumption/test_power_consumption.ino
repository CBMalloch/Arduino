...
void setup(){
  ...
  pinMode ( pdIndicator, OUTPUT ); digitalWrite ( pdIndicator, 1 );
  ...
  setup_watchdog ( 8 );  // arg sets the watchdog timeout interval
...
void loop(){
  ...
  */
  ...
  digitalWrite ( pdIndicator, 1 );
  // unsigned long t0 = millis();
  // Serial.print ( millis () - t0 ); Serial.print ( " : " );
  delay ( 10000 );                                             // I think this delay is incorrect
  // Serial.print ( millis () - t0 ); Serial.print ( " : " );
  digitalWrite ( pdIndicator, 0 );
  delay ( 10000 );
  // Serial.print ( millis () - t0 ); Serial.println();
  // delay ( 5 );
  digitalWrite ( pdAwake, 0 );
  
  // ***************** prepare to sleep, go to sleep
  
  pinMode ( pdIndicator, INPUT );      // set all used ports to input to save power
  pinMode ( pdAwake,     INPUT );      // set all used ports to input to save power

  wdt_reset();                         // restore the watchdog timer ( pet the doggy )S
  system_sleep();                      // we come back to the next line when we awaken

  pinMode ( pdIndicator, OUTPUT );     // reset set all ports back to pre-sleep states
