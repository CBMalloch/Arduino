...
void loop() {
  ...
  if (! digitalRead(pinSetpointDown)) {
    ...
    if (setpointButtonStatus != 0 && VERBOSE) {
      snprintf (strBuf, bufLen, "At %lu: new setpoint: %d/10 Deg\n",  
        msNow, 
        int(setpointDegrees * 10.0));
      Serial.print (strBuf);
    }
    setpointButtonStatus = 0;
    ...
    controller->setpoint((setpointDegrees - 32.0) * 5 / 9 + 273.15);
  }
  // ===================
  // measure and control
...
void doDisplay () {
  ...
  if ((msNow > tLastUpdate + loggingInterval) || (msNow < tLastUpdate)) {
    tLastUpdate = msNow;
    ...
    File dataFile = SD.open("PIDcrock.txt", FILE_WRITE);
    // if the file is available, write to it:
    if (dataFile) {
      dataFile.print(strBuf);
      dataFile.close();
    } else {
      Serial.println("error opening PIDcrock.txt");
    } 
  }
